...
The repeated Pixel 8 runs do not justify framework/runtime changes.

Static vs ECS: `ecs` is about 5 ms slower than `static` by median build time in
this matrix, but the ranges overlap and both modes are far slower than the
instanced path. This supports treating the current object ECS as acceptable
until a real workload proves otherwise.

`mountOnly` vs full sync: `mountOnly` is about 1 ms over `static` by median
build time, while full `ecs` is about 4 ms over `mountOnly`. The isolated
`scene.syncTransform` profiler average is only about 0.375 ms for 1,600
entities, so the larger frame-timing delta should be treated as device/run
variance and end-to-end scene overhead, not as proof that transform arithmetic
is the dominant cost.

Instancing is the clear lever for compatible groups. It cuts median build time
from 21-26 ms for per-node paths to 3.62 ms for the same visible cube count.

Changed-only transform sync is not the main lever for this benchmark. It can be
useful for mostly static scenes or very large transform counts, but it cannot
remove the per-node scene/render traversal that dominates the Pixel 8 captures.

The next RTS-shaped render benchmark should keep these separations: logic-only
ECS cost, `SceneNodeRef` mount cost, transform sync cost, per-node render cost,
and instanced-group render cost. The current `rts_workload_benchmark.dart`
already covers headless movement, state, selection, spatial-grid, and churn
logic; it is not a substitute for on-device rendering.

## Caveats

- Desktop `dart run` benchmarks run under the JIT. Use compiled AOT
  executables before comparing pure-Dart results over time.
...
- On-device conclusions should use repeated, alternating runs. Single runs are
  useful for smoke tests, not decisions.
