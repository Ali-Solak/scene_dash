  ...
  void build(AppBuilder app) {
    app
      ...
      ..addSystem(
        ...
        label: PlayerSystems.spawn,
      )
      ..addSystem(
        const ReadPlayerInputSystem(),
        schedule: Schedules.fixedPrePhysics,
        label: PlayerSystems.readInput,
      )
      ..addSystem(
        const MovePlayerSystem(),
        schedule: Schedules.fixedPrePhysics,
        label: PlayerSystems.move,
        after: const [
          PlayerSystems.readInput,
        ],
      );
  }
}
...
Do not replace this with function plugins unless explicitly requested.

## System

Systems are classes with generated parameter injection:
...
The `run()` method must remain synchronous.

Do not require systems to retrieve queries or resources manually from a context or service locator.
...
Generated system adapters resolve queries and resources during initialization:

```dart
final class $MovePlayerSystemAdapter
    implements SystemAdapter {
  final MovePlayerSystem system;

  late final Query2<Transform, Velocity>
      players;

  late final FixedTime time;

  $MovePlayerSystemAdapter(this.system);

  @override
  void initialize(World world) {
    players = world.createQuery2<
        Transform,
        Velocity
    >(
      writes: const [Transform],
      withTypes: const [Player],
      withoutTypes: const [Disabled],
    );

    time = world.resource<FixedTime>();
  }

  @override
  void run() {
    system.run(players, time);
  }
}
```

All stores, queries, resources, and event handles must be resolved before frame execution begins.
...
System ordering uses labels and dependency edges:
